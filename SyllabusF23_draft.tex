...
(from previous syllabus) This course introduces students to widely-used procedures for regression analysis and provides intuitive, applied, and formal foundations for more advanced methods treated in later courses. The course covers descriptive inference with regression from a modern perspective and will also introduce basic concepts in causal inference. While the course will emphasize the mathematical foundations of these concepts, each topic will also cover the implementation of the relevant methods in the statistical program R.
...
\item Distinguish between descriptive and causal interpretations of regression findings, and understand the assumptions that are necessary for each.
\end{itemize}
