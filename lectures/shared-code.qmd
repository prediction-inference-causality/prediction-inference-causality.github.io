...
percentpoint = label_percent(accuracy=.1)
approx  = function(x,accuracy=.01) { 
    label_number(accuracy=accuracy, scale_cut=cut_short_scale())(x) 
}
thousand = function(x, accuracy=.1) {
    approx(round((x/1000) / accuracy) * 1000 * accuracy, accuracy)
}
...
\newcommand{\Deltaall}{\Delta_{\text{all}}}
\newcommand{\mua}[1]{\ingroupa{\mu(0, #1)}}
\newcommand{\mub}[1]{\ingroupb{\mu(1, #1)}}
\newcommand{\pa}[1]{\ingroupa{p_{#1 \mid 0}}}
\newcommand{\pb}[1]{\ingroupb{p_{#1 \mid 1}}}
\newcommand{\pall}[1]{\inpurple{p_{#1}}}
\newcommand{\hmua}[1]{\ingroupa{\hat\mu(0, #1)}}
\newcommand{\hmub}[1]{\ingroupb{\hat\mu(1, #1)}}
\newcommand{\tmua}[1]{\ingroupa{\tilde\mu(0, #1)}}
\newcommand{\tmub}[1]{\ingroupb{\tilde\mu(1, #1)}}
$$
:::
...
width_binom = function(theta, n , alpha=.05) { 
    width_cdf(\(x)pbinom(n*x, n, theta), theta, c(0,1), alpha)
}
```
...
with_annotations = function(samples,
    ...
    vlines = if(is.na(vline.alpha)) geom_blank() else { 
        list(geom_vline(aes(xintercept=mu), color="blue", data=summary.data, alpha=vline.alpha),
             geom_vline(aes(xintercept=mu-w66/2), color="blue", linetype='dashed', data=summary.data, alpha=vline.alpha),
             geom_vline(aes(xintercept=mu+w66/2), color="blue", linetype='dashed', data=summary.data, alpha=vline.alpha), 
             geom_vline(aes(xintercept=mu-w95/2), color="blue", linetype='dotted', data=summary.data, alpha=vline.alpha),
             geom_vline(aes(xintercept=mu+w95/2), color="blue", linetype='dotted', data=summary.data, alpha=vline.alpha))
    }
