...
\begin{itemize}
\item For the last few lectures we have focused on linear regression coefficients.
...
\item Even with a binary covariate of interest, polynomials for the controls and interactions make the goal of linear regression unclear. 
\item Average Partial Derivatives are an important approach (analogous to the ``average over people'' estimator from earlier in the term).
\end{itemize}
