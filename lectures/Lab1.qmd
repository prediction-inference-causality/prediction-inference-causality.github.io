...
sam = pop[1:N, ]
sam.plot = ggplot(sam, aes(x=j,y=y)) + geom_point()
```
...
To do that, I'm going to use the function *map_vec* from the package **purr**.
...
### Functional Programming Convention (very optional)
...
- This is a built-in function most modern lanuages. Python, Julia, Scheme, Haskell, ML, etc. 
  ...
  - I tend to use the **purr** version just because the name is better.
  - People tend to speak the code above this way: 'map [the function] plus.one over the vector zero through nine'
- We're using it for a reason that's common but not the 'usual reason' people do.
  ...
  - So we'll pass it a lsit with the same thing repeated a bunch of times.
...
```{webr-r}
estimate = NaN                                         # NaN stands for Not a Number.
sam.plot + geom_hline(yintercept=estimate)             # Change it to a number.
```

### Again and Again

```{webr-r}
with.replacement = function(pop) {
  N = round(m * sampling.rate) 
...
without.replacement = function(pop) {
  N = round(m * sampling.rate) 
...
```{webr-r}
# editor-code-line-numbers: 2
randomization.then.sampling = function(pop) {
  flips = rbinom(m, 1, sampling.rate)
...
all.samples = pops |> map(function(...) {
  data.frame(with.replacement=with.replacement(pop),
             without.replacement=without.replacement(pop),
             by.randomization=by.randomization(pop),
             by.shuffling=by.shuffling(pop),
             randomization.then.sample=randomization.then.sampling(pop))
}) |> bind_rows() |> 
