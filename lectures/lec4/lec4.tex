...
\frame{
\frametitle{2020 Turnout in Georgia i.i.d. sample}
...
\begin{itemize}
\item We now need the sampling distribution of the difference in subsample means.
\item Again, we repeatedly sample with replacement individuals and their $(Y_i,X_i)$ variable pairs from the population, and calculate the difference in proportions.
\end{itemize}
}
...
\frame{
\frametitle{Connection to Linear Regression Slopes}

\begin{itemize}
\item Recall that in the first lecture, we showed that a difference in means/proportions can be parameterized as the slope coefficient from a linear regression with a single binary covariate. \pause
\item We can use this concept to re-write the sampling distribution of difference and bootstrap code for differences. \pause
\item On the next two slides, see if you can fill in the ? marks. 
\end{itemize}
...
\frametitle{Outputs from the lm() function}
\small
\begin{verbatim}
...
\begin{frame}[fragile]
\frametitle{Estimate, Std. Error, and t value}

We've already seen that the estimate 0.07 is the difference in means/proportions.\\ \pause

\bigskip

The Std. Error ("estimated standard error") can be calculated as the standard deviation from bootstrapped sampling distribution of the difference.

\begin{verbatim}
sd(bootdiffsamMean)
0.03953243
\end{verbatim} \pause
This a bit different (more accurate) from the bootstrapping, because the lm function assumes the same conditional variance for both values of the covariate. \pause
...
\begin{frame}[fragile]
\frametitle{Stars and Dots}
\small
Stars and Dots indicate different levels of significance:
\begin{itemize}
\item If the 90\% CI doesn't cover 0, but the 95\% does, it gets a dot
\item If the 95\% CI doesn't cover 0, but the 99\% does, it gets a star
\item If the 99\% CI doesn't cover 0, but the 999\% does, it gets two stars
\item If the 999\% CI doesn't cover 0, it gets three stars
\end{itemize}
...
Somewhere close to 0.072. Again a little different than from the table due to boostrapping producing a better p-value. 
