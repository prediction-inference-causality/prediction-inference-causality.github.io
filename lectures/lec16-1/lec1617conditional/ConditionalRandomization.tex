...
\date{November 14 and 16, 2017}%[Short Occasion] % (optional)
...
\section{Conditionally Randomized Experiments}
...
With a conditionally randomized experiment, the (marginal) exchangeability/unconfoundedness assumption does not hold.
...
\frame{
\frametitle{Estimation with Conditionally Randomized Experiments}
...
\frame{
\frametitle{Interpretation with Conditionally Randomized Experiments}
...
&= \beta_1
\end{align*}
can be interpreted as the ATE among the $X=0$ subjects. \pause
\begin{align*}
E[Y|T=1,X=1]&-E[Y|T=0,X=1] = \\
&(\beta_0 + \beta_1 + \beta_2 + \beta_3) - (\beta_0 + \beta_2) \\
&= \beta_1+\beta_3
\end{align*}
can be interpreted as the ATE among the $X=1$ subjects. \pause
\begin{align*}
...
\frame{
\frametitle{Statistical Inference with Randomized Experiments}
...
If $\beta_3 \ne 0$, and the four descriptive assumptions hold  then we can get approximate CIs and tests for the ATE by subtracting the mean $E[X]$ off of all $X$ values, running the interactive regression of $Y$ on $T$, $X-E[X]$, and $T\cdot (X-E[X])$ and using the estimate and standard error associated with coefficient on $T$.
