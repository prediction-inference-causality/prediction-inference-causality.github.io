...
scatter = ggplot() + geom_point(aes(x=W, y=Y), color="red", data=tibble(W=W,Y=Y),
           ...
           geom_pointrange(aes(x=W, y=mean, ymin=mean-2*sd/sqrt(n), ymax=mean+2*sd/sqrt(n)), data=summaries, alpha=.8) +
...
- Randomization reduces this to estimating a difference in subpopulation means.
- And we try estimating that using a difference in subsample means. I've drawn these, $\pm$ two standard errors, in black.

$$
\tau(100) = \frac{1}{m_{100}}\sum_{j:w_j=100} y_j - \frac{1}{m_{50}}\sum_{j:w_j=50} y_j 
          \approx \hat\tau(100) = \frac{1}{N_{100}}\sum_{i:W_i=100} Y_i - \frac{1}{N_{50}}\sum_{i:W_i=50} Y_i
$$          
...
n = 250
W = sample(c(50,100), size=n, replace=TRUE) 
Y = case_match(W,
  0~Y0,
  50~Y50,
  100~Y100)

summaries.new = tibble(W=W, Y=Y) %>% group_by(W) %>% summarize(mean=mean(Y), sd=sd(Y), n=length(Y))
point.estimate = summaries.new$mean[summaries.new$W==100] - summaries.new$mean[summaries.new$W==50]

scatter.new = ggplot() + geom_point(aes(x=W, y=Y), color="red", data=tibble(W=W,Y=Y),
                                     position=position_jitter(w=10,h=0), alpha=.3) +
           geom_pointrange(aes(x=W, y=mean, ymin=mean-2*sd, ymax=mean+2*sd), data=summaries) +
           scale_x_continuous(breaks=c(0,50,100)) + labs(x="W", y="Y") + ylim(-3,15)
scatter.new
```
...
```{r}
boots.ifwecould = boot.distn(tibble(W=W, Y=Y))
```
