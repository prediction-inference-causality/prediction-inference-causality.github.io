...
- To sum over roll sequences, we sum over one roll after another.
- Suppose we're calculating the probability of the response sequence $a_1 \ldot a_n = 0,0,...,0,0$.
    1. We consider any 'head' $j_1\ldots j_{n-1}$ where $y_{j_1} \ldots y_{j_{n-1}}=0,0,...0$ and think about choosing the last element.
...
$$
\begin{aligned}
\sum_{\substack{j_1 \neq \ldots \neq j_n  \\ y_{j_1} \ldots y_{j_n} = 0 \ldots 0}} \frac{m-n!}{m!} 
&= \sum_{\substack{j_1 \neq \ldots \neq j_{n-1} \\ y_{j_1} \ldots y_{j_{n-1}} = 0 \ldots 0}} \sum_{\substack{j_n \not\in j_1 \ldots j_{n-1} \\ y_n=0}} \frac{(m-n)!}{m!} \\
&= \sum_{\substack{j_1 \ldots j_{n-1} \\ y_{j_1} \ldots y_{j_{n-1}} = a_1 \ldots a_{n-1}}} \sum_{\substack{j_n \not\in j_1 \ldots j_{n-1} \\ y_n=0}} (m_{0} - n + 1) \times \frac{(m-n)!}{m!} \\
&= \sum_{\substack{j_1 \neq \ldots \neq j_{n-2} \\ y_{j_1} \ldots y_{j_{n-2}} = 0 \ldots 0}} \sum_{\substack{j_{n-1}\not\in j_1 \ldots j_{n-2} \\ y_n=0}}  (m_0 - n + 2) (m_{0} - n + 1) \times \frac{(m-n)!}{m!} \\
&= m_0 \times \ldots \times (m_0 - n + 1) \times (m-n)!/m! = \frac{m_0!}{(m_0-n)!} \times \frac{(m-n)!}{m!} \qqtext{after repeating $n-2$ more times} \\
&= \frac{m_0!}{(m_0-s_0)!} \times \frac{m_1!}{(m_1-s)!} \times \frac{(m-n)!}{m!}  \qqtext{ generally. }
\end{aligned}
$$
...
$$
\begin{aligned}
\sum\limits_{\substack{j_1 \ldots j_n \\ y_{j_1} + \ldots + y_{j_n} = s}} (m-n)!/m!
&= \sum\limits_{\substack{a_1 \dots a_n \\ a_1 + \ldots + a_n = s}} \ \ \sum_{\substack{j_1 \ldots j_n \\ y_{j_1} \ldots y_{j_n} = a_1 \ldots a_n}} (m-n)!/m! \\
& =\sum\limits_{\substack{a_1 \dots a_n \\ a_1 + \ldots + a_n = s}} \frac{m_0!}{(m_0-s_0)!} \times \frac{m_1!}{(m_1-s_1)!} \times \frac{(m-n)!}{m!}  \\
&= \binom{s}{n} \frac{m_0!}{(m_0-s_0)!} \times \frac{m_1!}{(m_1-s)!} \times \frac{(m-n)!}{m!}
\end{aligned} 
$$
...
```{r}
m = 7.2e6
with.vs.without = function(m,theta,n) {
    theta = round(theta*m)/m
    probs = data.frame(
      x=0:m,
      hyper=dhyper(0:m, theta*m, (1-theta)*m, n),
      binom=dbinom(0:m, n, theta),
      norm=dnorm(0:m, theta*n, sqrt(n*theta*(1-theta))))
    
    relevant = probs$hyper > 1e-6
    ...
    whyper = width_cdf(\(x)phyper(x*n, theta*m, (1-theta)*m, n), center=theta, limits=c(0, 1), alpha=.05)
    wbinom = width_cdf(\(x)pbinom(x*n, n, theta), center=theta, limits=c(0, 1), alpha=.05)
    wnorm  = width_cdf(\(x)pnorm(x*n, theta*n, sqrt(n*theta*(1-theta))), center=theta, limits=c(0, 1), alpha=.05)
    
    sample.data = data.frame(x = rhyper(10000, theta*m, (1-theta)*m, n)/n,
                             y = seq(0, 1, length.out=10000)*max(probs$hyper))
    dy = .005*max(probs$hyper)
    
    plot.sample.data = sample.data[seq(1,10000,by=100), ]
    
    plot = ggplot(probs[relevant, ]) + 
      geom_area(aes(x=x/n, y=hyper), color='blue', alpha=.2) +
      geom_area(aes(x=x/n, y=binom), color='red', alpha=.2) + 
      geom_pointrange(aes(x=x,y=y,    xmin=x-whyper/2, xmax=x+whyper/2), data=plot.sample.data, color='blue', alpha=.2) +
      geom_pointrange(aes(x=x,y=y+dy,xmin=x-wbinom/2, xmax=x+wbinom/2),  data=plot.sample.data, color='red', alpha=.2)
    
    coverage.norm = mean( theta-wnorm/2 <= sample.data$x & sample.data$x <= theta+wnorm/2)
    coverage.binom = mean( theta-wbinom/2 <= sample.data$x & sample.data$x <= theta+wbinom/2 )
    coverage.hyper = mean( theta-whyper/2 <= sample.data$x & sample.data$x <= theta+whyper/2 )
    list(plot=plot, coverage.binom=coverage.binom, coverage.hyper=coverage.hyper)
}
```
...
theta = mean(y)
same       = with.vs.without(m, theta, n)
different  = with.vs.without(m, theta, round(m/2))

same$plot
different$plot
```

- When we're sampling a small fraction of our population, e.g. `r n`/`r approx(m)`, not much.
...
with [sampling without replacement]{.fg style="color:blue"} than [sampling with replacement]{.fg style="color:red"}
    - In this case, acting as if you'd sampled with replacement would give you wider-than-necessary intervals.
    - You get 95% coverage either way. [`r percent(different$coverage.binom)`]{.fg style="color:red"} vs. [`r percent(different$coverage.hyper)`]{.fg style="color:blue"}.
