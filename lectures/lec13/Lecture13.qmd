...
## We Talked About Estimating An Average Treatment Effect
...
$$
\hat\tau(100) = \hat\mu(100) - \hat\mu(50) \qqtext{where} \hat\mu(x) = \hat a x + \hat b \qfor \hat a,\hat b = \argmin_{a,b} \sum_{i=1}^n \qty(ax + b - Y_i)^2
$$ 
...
line.sampling.dist = ggplot() + 
  ...
  geom_pointrange(aes(x=slope.point.estimate, y=dnorm(2*slope.se, mean=0, sd=slope.se), 
                      ...
                      xmax=slope.point.estimate + 1.96*slope.se)) + scale_x
line.sampling.dist
``` 
...
line.sampling.dist + geom_pointrange(aes(x=estimate, y=(8*i/100)*dnorm(2*slope.se, mean=0, sd=slope.se), 
                      ...
                      xmax=estimate + 1.96*se), data=line.based.estimates[1:100,], color='purple', alpha=.4) + scale_x 
```
