---
title: "Lab 3"
subtitle: "Probability Background: Expectations"
...
- The most common measure is the *variance*, which is the expectation of the squared distance from the mean.
  ...
  - Other measures like this might be the *median absolute difference* from *the median*. But we won't bother with those.
...
- Criticism 2. Why do we care about a random variable anyway? We care about the *population* from which it's drawn.
- Any comments in support of expectations and variances? Maybe responses to these criticisms?

### Re: Criticism 2

- If a random variable $Y$ is *equally likely* to be any member of a population $y_1 \ldots y_m$, then **its Expectation is the population mean**.

$$
\E[Y] = \sum_{j=1^m} \underset{response}{y_j} \times \underset{probability}{\frac{1}{m}} = \frac{1}{m}\sum_{j=1}^m y_j = \bar y
$$
...
(a+b)^2 = a^2 + 2ab + b^2
$$
How does that generalize to a bigger sum?
$$
