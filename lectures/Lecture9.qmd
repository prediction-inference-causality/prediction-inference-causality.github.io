...
scatter.by.group = function(data, dot=sam.dot, width=sd, 
  ...
  if(!is.null(line.alpha)) { 
    p = p + stat_summary(aes(x=education, y=income, color=group), 
  ...
  p + geom_point(aes(x=xco(group, education),  y=income, color=group), 
      ...
      stat_summary(aes(x=xco(group, education,shift=.025), y=income, color=group), geom="pointrange", alpha=.7, fun.data=mean_sd)  +
...
- We've counted the one who applied to Department 0 $\faint{3 \times}$  as much as the ones who applied to Department 1.
...
- We can, in fact, think of this as the average difference between ...
...
```{r}
berkeley.with.histograms +
  geom_bar(aes(x=x, y=after_stat(prop)), alpha=.1, linewidth=.1, fill='purple', color='purple')
```
...
::::
  ...
  - **Part 1.** For each of these summaries, describe the group we're averaging over. Do it three ways.
    ...
    2. As you'd describe the dots in the plot, e.g. 'allgreen dots in the x=1 column'.
...
- It's common to use the same words to describe these three different summaries.
  ...
  - Or sometimes '..., *controlling for department*'.^[This can be confusing because it has a different --- but related --- meaning when when we're talking about designing an experiment.]
...
When we look at the acceptance rates to the graduate school in 'histogram form', \
...
\begin{aligned}
\hat\Delta_{\text{raw}}
&=\text{difference in acceptance rates for women and men for the Graduate School as a whole} \\ 
...
&= \underset{\text{within-department summary}}{\sum_{x} \ingreen{P_{x\mid 1}} \ \{\ingreen{\hat\mu(1,x)} - \inred{\hat\mu(0,x)}\} } 
...
- It differs by a 'covariate shift term'. 
  - This term that has nothing to do with the disparity within departments.
...
- In our paradox example, this term is negative.
  - The graduate school-level difference is smaller than our summary of the within-department ones.
  - It's negative while the within-department differences are positive.^[Remember we're looking at [rates for women]{.groupb} - [rates for men]{.groupa}.]
- In our warm-up example, this term was *the whole difference* we saw on the university level. 
...
- The covariate shift term is the difference of two averages of the same function of x.
    ...
    - The term is the difference of two averages.
      - The average over green dots or equivalently the average weighted by green bars. Women's department choices.
      - The average over red dots or equivalently the average weighted by red bars. Men's department choices.
- Why is it negative? 
  - The function we're averaging *decreases from left to right*. 
  - The distribution we're averaging over *shifts from [left]{.groupb} to [right]{.groupa}*.
...
- There's a simple heuristic that'll tell you if this term is positive or negative.
  - It's like multiplying two numbers: $+1 \times +1 = +1, +1 \times -1 = -1, -1 \times +1 = -1, -1 \times -1 = +1$.
  - The two factors here are our function and a distribution shift. 
- To use this heuristic, we need to give these two factors signs. 
  ...
  - A distribution shift is 'positive' if probability mass shifts from left to right and 'negative' if  it shifts from right to left.
$$
\overset{-1}{\text{decreasing function}} \times \overset{+1}{\text{rightward shift}} = \overset{-1}{\text{negative difference}}
$$

- This'll be more useful but subtler when we have more levels of $X$.
...
scaled.interleaved.bars = list(geom_bar(aes(x=xco(group, education), y=proportion.scale*after_stat(prop), fill=group, color=group), 
                               position='identity', alpha=.03, linewidth=.1), 
