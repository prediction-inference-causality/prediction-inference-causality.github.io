...
- But it does tell you something if you look at it right. 
  ...
  - How often is it off by $\inteal{1/3}$ or less?   `r percent(mean(abs(estimates - mean(y)) <= 1/3+.01, na.rm=TRUE))` of the time.
- To make this sound like a statement about the population, we report two things.
  1. An **interval** you can expect the population proportion to be in. 
    - e.g., we think it's in the interval $\inmidnight{2/3} \pm \inteal{1/3}$ because $\inmidnight{2/3}$ is our sample proportion.
...
- It's tempting to say something nonsensical.
  - e.g. "the turnout frequency in the election is in my confidence interval, `r round(mean(Y)-w/2,2)`-`r round(mean(Y)+w/2,2)`, 95% of the time." 
