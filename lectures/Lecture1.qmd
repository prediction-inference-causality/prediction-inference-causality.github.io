...
```{r}
j.plot = sample(1:nrow(pop))[1:23000]
...
::: {.fragment .current-visible data-fragment-index="0"}
```{r}
estimates.plot + faint.intervals + strong.interval + scales
```
:::
...
estimates.plot +
    strong.interval + 
    geom_histogram(aes(x=rate, y=3*after_stat(density)), alpha=.21, data=bootstrap.reps, binwidth=.2) +
    geom_point(aes(x=rate, y=y), data=bootstrap.reps, color='purple', alpha=.2) + 
    scales
```
:::
...
estimates.plot +
    strong.interval + 
    geom_histogram(aes(x=rate, y=3*after_stat(density)), alpha=.22, data=bootstrap.reps, binwidth=.2) +
    geom_point(aes(x=rate, y=y), data=bootstrap.reps, color='purple', alpha=.2) + 
    scales
```
:::
...
- Making statements about our estimand using our sample is called *inference*.
  ...
  - Usually a range centered on a *point estimate* like our [green dot]{.groupa}.
