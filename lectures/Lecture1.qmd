...
- That way, when we count up our predictions for 100 people like Rush, we get `r round(100*mean(rush.votes),2)` Yeses. Not zero.
  - And when we do it for `r length(rush.votes)` people like Rush, we get `r sum(rush.votes)`. 
  - `r length(rush.votes)` is the number of [green dots]{.groupb} like Rush in the experiment and `r sum(rush.votes)` is the number of Yeses among them.
...
- Let's imagine that we really did mail the letter to every voter in Michigan.
 ...
 - If we've drawn a range --- *an interval estimate* --- we'd want to know whether the population rate is in there.
  ...
  - But we might ask how often we'd get an interval that contains the population rate if ...
    - ... we drew samples of `r sum(social.pressure$treatment==' Neighbors')` voters the same way, over and over, 
    - ... each time rolling our die `r sum(social.pressure$treatment==' Neighbors')` times to select `r sum(social.pressure$treatment==' Neighbors')` people 
...
::: {.fragment .current-visible data-fragment-index="2"}
 ...
 - What we'd be doing is thinking of our estimate as the result of a procedure we could apply to any sample.
  ...
  - If we applied that procedure to 100 different samples, we'd get [100 different numbers]{.purple}. Like our [purple dots]{.purple} ([`r circle`]{.purple}s).
- We don't have a bunch of samples like the ones we'd want. We'd have to time-travel to 2006 and mail letters.
- But it turns out we can fake it well enough to get by. That's how we got the [`r circle`]{.purple}s.
...
- This gives you a sense of how much of your error is a matter of chance. 
  ...
  - ... to have gotten any of those [`r circle`]{.purple}s as the [`r circle`]{.groupb} that you did.
...
- If you twist that just right, we can make a meaningful claim about where that center is.
  ...
  - ... then, turning that around, that center has to be pretty close to [the sample]{.groupb} we do have.
:::
::: {.fragment .current-visible data-fragment-index="5"}


- The [interval estimate]{.groupb} I've drawn is actually *calibrated* using this idea.
- Its width is chosen so that, unless the [`r circle`]{.groupb} is an unusually off-center draw from the real distribution ...
...
### Practice working with Distribution Plots
 How many of the [`r circle`]{.purple}s are estimates of at least 30%? What about 29%?
...
- Does the sample of [`r circle`]{.groupb}s we got *look like* one we'd get if we'd mailed the letter to everyone?
