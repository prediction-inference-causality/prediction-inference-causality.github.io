...
- I'd love to look at the real data, but we don't have it.
  - I contacted one of the authors of `r kim2023` and they couldn't give it to me. - That's almost always the case with medical records. It's a patient privacy thing. 
...
- To make all this feel a little familiar, we're going to look at some fake data.
...
$$
\frac{\frac{46.5\%}{1-46.5\%}}{\frac{47.1\%}{1-47.1\%}} \approx  `r round(46.5/(1-46.5) / 47.1/(1-47.1), 2)` \qqtext{ is less than 1 } 
$$
...
- Even if the odds ratio *were* constant --- i.e. if the 'unless' part was true --- misspecification is likely.
  ...
  $$
  \mu(w,x)=\invlogit(a(w) + b(x)) \iff \log\left\{\frac{\mu(w,x)}{1-\mu(w,x)}\right\}=a(w) + b(x)
  $$
  - Our model includes a small subset of functions like that: the ones where the log odds are linear combinations of $x_1 \ldots x_6$. 
  $$ 
  m(w,x) = \invlogit(a_0 + a_w w + b_1 x_1 + \cdots + b_6 x_6) \iff 
...
- We get similar point estimates when we calculate the ATE and ATT using the other models we've looked at.
