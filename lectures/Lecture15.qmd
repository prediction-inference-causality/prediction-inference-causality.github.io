...
#|   - x
#|   - trim_level 
sam1 = sam; sam1$x = sam1[,x] 
gamma.att.thing = sbw.att(sam1, K.gaussian)(1)
gamma.att.sbw = \(sam) gamma.att.thing(sam[,x])
...
estimators = function(sample) {
  data.frame(ipw=estimate.att(sample, formula, gamma=trimmed(gamma.att, trim_level)),
...
```{webr}
#| autorun: true
K.seminorm = function(x,xp, s=1) {
    diff.norm = abs(x-xp)
    (-1)^(s+1) * ifelse(diff.norm==0, 0, diff.norm^(2*s)*log(diff.norm))
}
P.seminorm = function(x,s=1) {
    outer(x,0:(s-1), function(x,p) x^p)
}
...
```{webr}
#| autorun: true
predict.kernel.regression = function(obj, newdata, KxX=NULL, Px=NULL) {
    x = newdata$X
    X = obj$X
    K = obj$K
    P = obj$P
    ...
    if(is.null(KxX)) { KxX = outer(x,X,K) }
    if(is.null(P)) { 
        KxX %*% gamma 
    } else {
...
```{webr}
#| autorun: true
ridge.both = function(W,X,Y,K,P=NULL) {
    KxX = outer(X,X[W==1],K)
    KXX = KxX[W==1,]
    ...
    gamma = function(sigma) { 
        if(!is.null(P)) {
            alpha = solve(t(PX) %*% PX  - t(VtPX) %*% diag(1/(l + sigma^2)) %*% VtPX,
                          colSums(Px) -   t(VtPX) %*% diag(1/(l + sigma^2)) %*% VtsumKxX)
            VtsumKxX = VtsumKxX - VtPX %*% alpha
        }
        gamma = V %*% diag(1/(l^2 + sigma^2*l)) %*% VtsumKxX 
        obj = list(alpha=alpha, gamma=gamma, K=K, P=P, X=X[W==1])
        attr(obj, 'class') = 'kernel.regression'
        obj
    }
    
    list(gamma=gamma, evd=evd, KxX=KxX, Px=Px) 
}
```
...
sbw.att = function(sam, K=K.gaussian) {
  fits = ridge.both(sam$w, sam$x, sam$y, K.gaussian)
  function(sigma) { 
    gamma0 = predict(fits$gamma(sigma), newdata=data.frame(X=sam$x))
    ifelse(sam$w==1, 1, gamma0)
  }  
}
```
:::
