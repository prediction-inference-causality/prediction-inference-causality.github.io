...
## The Interacted Bivariate Linear Model's Sampling Distribution
...
- We may not be able to fit the data at all.
- And our slope will be some mix of ...
...
## The Additive Bivariate Linear Model's Sampling Distribution
...
- This is the same as first coarsening the data --- forgetting age differences within bins --- then fitting a saturated model. But people do keep their (uncoarsened) weights.
...
## Why Does Going Big Not Increase Variance (Much)?
...
plot.image = function(im) { 
  ...
  ggplot(im) + geom_raster(aes(x=x, y=y, fill=z)) + xlim(0,1) + ylim(0,1)
}
...
set.seed(1)
sampled.leak = sample.px(leak, 5000, density.im$z, noise.level=.1)
...
- Suppose we're interested in the health impacts of a chemical leak.
    ...
    - Lighter colors mean higher concentrations.
- That's our population. But we can't measure concentration everywhere.
- We'll measure it a set of randomly chosen locations. 
...
- We'll take them at random times, too---there are many observations $(z_j,x_j,y_j)$ at the same location $(x_j,y_j)$ in the population we're sampling from. We'll sample with replacement. 
- The probability a sample is at a given location is shown on the right.
:::
...
plot.image(leak)
plot.predictions(sampled.leak, z~x+y)
```
...
#| fig-asp: 1
#| layout-ncol: 2
plot.predictions(sampled.leak, pc.formula.2d(seq(0,1, length.out=3+1), seq(0,1, length.out=3+1)))
plot.predictions(sampled.leak, pc.formula.2d(seq(0,1, length.out=6+1), seq(0,1, length.out=6+1)))
```
...
#| fig-asp: 1
#| layout-ncol: 2
plot.predictions(sampled.leak, pc.formula.2d(seq(0,1, length.out=9+1), seq(0,1, length.out=9+1)))
...
- On the left, we have $9 \times 9 = 81$ bins; on the right, $12 \times 12 = 144$.
...
#| fig-asp: 1
#| layout-ncol: 2
plot.predictions(leak, pc.formula.2d(seq(0,1, length.out=9+1), seq(0,1, length.out=3+1)))
plot.predictions(sampled.leak, pc.formula.2d(seq(0,1, length.out=3+1), seq(0,1, length.out=9+1)))
```

- We can try using rectangles, rather than squares, as bins.
- Both of these have $9 \times 3 = 27$.
