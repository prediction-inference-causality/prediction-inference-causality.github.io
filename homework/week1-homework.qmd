...
theta.hat
```
1. This is the only line of code that's different.

The estimate is `r theta.hat`. To get it, you can just rerun all the code above after making the one-line change indicated by the `(1)` mark. The result should look like this. Different looking sample. Way more big-scorers.
...
sampling.distribution.histogram +
  geom_point(aes(x=mean(your.sample$PTS), y=.003), color="red")
```
...
::: {.callout-tip collapse="true"}
### Code You'll Want
Here's some code that'll be useful.^[We'll get into what the code is doing in next week's homework. But if you want to know now, go ahead and give it a read. See if you can work it out.] It takes a list of numbers---e.g. draws from our sampling distribution---and gives you a width. It finds the width you need for an interval centered at the list's mean to cover all but $100\alpha\%$ of its elements. Like the green shaded region we found in [Lecture 2](https://qtm285-1.github.io/assets/lectures/Lecture2.html#/using-the-sampling-distribution-for-calibration-1). If you have a list `draws` of draws from your estimator's sampling distribution, you can say `width(draws)` to get the right width for a 95% confidence interval.
...
w = width(estimates)
my.theta.hat = mean(my.sample$PTS)
your.theta.hat = mean(your.sample$PTS)

sampling.distribution.histogram +
  geom_pointrange(aes(y=.004,  x=my.theta.hat,   xmin=my.theta.hat   - w/2, xmax=my.theta.hat   + w/2), color="blue") +
  geom_pointrange(aes(y=.004,  x=your.theta.hat, xmin=your.theta.hat - w/2, xmax=your.theta.hat + w/2), color="red")
```

['My interval']{.fg style="color:blue"} is `r approx(my.theta.hat)` &plusmn; `r approx(w/2)` or equivalently [`r approx(my.theta.hat - w/2)`, `r approx(my.theta.hat + w/2)`]. ['Your interval']{.fg style="color:red"} is `r approx(your.theta.hat)` &plusmn; `r approx(w/2)` or equivalently [`r approx(your.theta.hat - w/2)`, `r approx(your.theta.hat + w/2)`]. Admittedly, the use of 'my' and 'your' here may be a bit confusing, so whatever you called them works for me.^[There's a bit in a 1997 episode of *The Simpsons* involving this you/me problem and Smokey the Bear's famous catchphrase 'Only you can prevent forest fires'. Thinking now about who---if anyone---really could prevent forest fires and who Smokey the Bear (and Captain Planet, etc.) were ultimately speaking for, it's pretty sharp for a joke
that works as inane grammatical bit for people who don't really get it.  Way over my head at age 11.]
:::
...
covers = pops |> map_vec(function(pop) {
  ...
  theta.hat = mean(sam$PTS)                            #<1>
  interval.estimate = c(theta.hat-w/2, theta.hat+w/2)  #<1>
  this.one.covers = interval.estimate[1] <= theta & theta <= interval.estimate[2]  #<2>
  this.one.covers
})
coverage.frequency = mean(covers)
```
1. I used my code from @exr-interval-estimate.
2. I like to write 'in-interval' checks like this as `a <= t & t <= b`, instead of
something equivalent like `t >= a & t <= b`, because it communicates what we're doing a bit better visually. 
`t` *is* between `a` and `b` in the expression `a <= t & t <= b`. In Python, you can even write it as `a <= t <= b`.
3. Remember this from @exr-interval-estimate?
4. Our estimation target. The thing we're trying to cover.
   
It's 95%. In 10,000 draws from our sampling distribution, `r 100*coverage.frequency`% of the intervals covered.
:::