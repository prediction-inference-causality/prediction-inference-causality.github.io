...
a1<- ggplot(data = df, aes(x = x, y = y)) +
  geom_point() + xlim(-10,15)+ylim(-30,45) + xlab("x") +
  ylab("y") +
  theme_bw()

a2<- ggplot(data = rdf, aes(x = rxz, y = ryz)) +
  geom_point() + xlim(-10,15)+ylim(-30,45) +  xlab("r_(x~z)") +
  ylab("r_(y~z)") +
  theme_bw()
...
covariatates uncorrelated: lm outputs 

simple plot, residualized plot 

what is the slope 

\subsubsection*{Part C}

relationship between standard errors 
